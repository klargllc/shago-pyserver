...
def get_simlar_items(sampleItem):
	items = FoodItem.objects.filter(
		Q(name__icontains=sampleItem.name)|
		Q(tags__icontains=sampleItem.tags)
	).exclude(name__iexact=sampleItem, id__iexact=sampleItem.id)
	return items
...
def paginate_items(items, request, num_per_page=1):
...
def menu_view(request):
	org  = request.GET.get('place', None)
...
@api_view(["GET"])
@required_params('foodId')
def item_detail_view(request, **kwargs):
	foodId = request.GET.get('foodId')
	food_item = FoodItem.objects.get(slug=foodId)
